...
INPUT_CONTAINER_NAME = 'input-files' # Container name
OUTPUT_CONTAINER_NAME = 'output-files' # Container name
...
class DataSetCache(object):
    ...
    def get(self, name):
        ...
        if result is None:
            logging.info("Error opening: " + NameError)
            sys.exit(1)
...
class tile_info(object):
    ...
    def report(self):
        logging.info('tileWidth:   ' + self.tileWidth)
        logging.info('tileHeight:  ' + self.tileHeight)
        logging.info('countTilesX: ' + self.countTilesX)
        logging.info('countTilesY: ' + self.countTilesY)
        logging.info('overlap:     ' + self.overlap)
    ...
    def getDataSet(self, minx, miny, maxx, maxy):
        ...
        for feature in features:
            ...
            for bandNr in range(1, self.bands + 1):
                ...
                if data is None:
                    logging.info(gdal.GetLastErrorMsg())
...
def createPyramidTile(levelMosaicInfo, offsetX, offsetY, width, height, tileName, OGRDS, feature_only):
    ...
    if t_fh is None:
        logging.info('Creation failed, terminating gdal_tile.')
        sys.exit(1)
...
def createTileIndex(dsName, fieldName, srs, driverName):
    ...
    if OGRDriver is None:
        logging.info('ESRI Shapefile driver not found')
        sys.exit(1)
...
def addFeature(OGRDataSource, location, xlist, ylist):
    ...
    if OGRFeature is None:
        logging.info('Could not create Feature')
        sys.exit(1)
    ...
    if OGRGeometry is None:
        logging.info('Could not create Geometry')
        sys.exit(1)
...
def UsageFormat():
    logging.info('Valid formats:')
    count = gdal.GetDriverCount()
    for index in range(count):
        driver = gdal.GetDriver(index)
        logging.info(driver.ShortName)



def retile(Verbose_ = False,
            Quiet_ = False,
            CreateOptions_ = [],
            Names_ = [],
            TileWidth_ = 256,
            ...
            Extension_ = None,
            MemDriver_ = None,
            TileIndexFieldName_ = 'location',
            TileIndexName_ = None,
            TileIndexDriverTyp_ = "Memory",
            CsvDelimiter_ = ";",
            CsvFileName_ = None,
            Source_SRS_ = None,
            TargetDir_ = None,
            ResamplingMethod_ = gdal.GRA_NearestNeighbour,
            Levels_ = 0,
            PyramidOnly_ = False,
            LastRowIndx_ = -1,
            UseDirForEachRow_ = False,
            Resume_ = False):
    ...
    MemDriver = MemDriver_
    TileIndexFieldName = TileIndexFieldName_
    TileIndexName = TileIndexName_
    CsvDelimiter = CsvDelimiter_
    CsvFileName = CsvFileName_
    TileIndexDriverTyp = TileIndexDriverTyp_
    Source_SRS = Source_SRS_
    TargetDir = TargetDir_
    ResamplingMethod = ResamplingMethod_
    Levels = Levels_
    PyramidOnly = PyramidOnly_
    UseDirForEachRow = UseDirForEachRow_
    Resume = Resume_

    if not os.path.exists(TargetDir):
        logging.info("TargetDir " + TargetDir + " does not exist")
        return 1
    ...
    if (TileWidth - Overlap <= 0 or TileHeight - Overlap <= 0):
        logging.info("Overlap too big w.r.t tile height/width")
        return 1

    if TargetDir is None:
        logging.info("Missing Directory for Tiles -targetDir")
        return 1
    ...
    if Levels > 0:  # prepare Dirs for pyramid
        ...
        for levelIndx in range(startIndx, Levels + 1):
            ...
            os.mkdir(leveldir)
            if not os.path.exists(leveldir):
                logging.info("Cannot create level dir: " + leveldir)
                return 1
            if Verbose:
                logging.info("Created level dir: " + leveldir)

    Driver = gdal.GetDriverByName(Format)
    if Driver is None:
        logging.info('Format driver not found, pick a supported driver. Format: ' + Format)
        UsageFormat()
        return 1
    ...
    if tileIndexDS is None:
        logging.info("Error building tile index")
        return 1
    ...
    if Source_SRS is None and minfo.projection:
        ...
        if Source_SRS.SetFromUserInput(minfo.projection) != 0:
            logging.info('invalid projection  ' + minfo.projection)
            return 1
